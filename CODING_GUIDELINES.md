...
The Keycloak UI projects uses best practices based off the offical [React TypeScript Cheatsheet](https://react-typescript-cheatsheet.netlify.app/), with modificiations for this project. The React TypeScript Cheatsheet is maintained and used by developers through out the world, and is a place where developers can bring togehter lessons learned using TypeScript and React.
...
For props we are using **type** instead of **interfaces**. The reason to use types instead of interfaces is for consistency betweeen the views and because it's more constrained (See [Types or Interfaces](https://react-typescript-cheatsheet.netlify.app/docs/basic/getting-started/types_or_interfaces) for more clarifiation). By using types we are ensuring that we will not deviate from the agreed upon [contract](https://dev.to/reyronald/typescript-types-or-interfaces-for-react-component-props-1408).
...
### Maintiaing state with types

When maintaining state for a component it is recommend that you use a [type instead of an interface](https://react-typescript-cheatsheet.netlify.app/docs/basic/getting-started/types_or_interfaces).
...
For components that do not have any additonal props an empty object should be used instead:
...
Additonal details around function components can be found [here](https://react-typescript-cheatsheet.netlify.app/docs/basic/getting-started/function_components).
...
Currently we recommend using inference for the primative types booleans, numbers, and string when using useState. Anything other then these 3 types should use a declartive syntax to specify what is expected. For example the following is an example of how to use inference:

```javascript
const [isEnabled, setIsEnabled] = React.useState(false);
```

Here is an example how to use a declaritive syntax. When using a declearative syntax if the value can be null that will also need to be specified:
...
The second option is for createing [mutable refs](https://react-typescript-cheatsheet.netlify.app/docs/basic/getting-started/hooks#useref) that you will manage.
...
Besides the details outlined above a list of recommendations for Typescript is maintained by several Typescript React developers [here](https://react-typescript-cheatsheet.netlify.app/). This is a great reference to use for any additonal questions that are not outlined within the coding standards.
...
// Data tables throughout kogito that are marked as compact 
...
.kogito-management-console--data-table--compact--lg {
...
}
```
...
For example, instead of using the utility class for margin-right multiple times, we should define a new Keycloak admin console class that adds  *margin-right: var(--pf-global--spacer--sm); *
And in this example, the new class can set the color appropriately as well.
...
switch (titleStatus) {
   case 'success':
     return (
       <>
         <InfoCircleIcon
           className="keycloak-admin--icon--info" // use a new kogito- class 
         />{' '}
         {titleText}{' '}
       </>
     );
...
* [PatternFly CSS utility classes](https://www.patternfly.org/v4/documentation/react/overview/react-training)
